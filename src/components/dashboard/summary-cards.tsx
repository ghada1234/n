    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t('netCalories')}</CardTitle>
          <Flame className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{netCalories}</div>
          <p className="text-xs text-muted-foreground">
            {summaryData.consumed.calories} {t('consumed')} - {summaryData.burned} {t('burned')}
          </p>
          <Progress
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t('carbs')} (g)</CardTitle>
          <Cookie className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{summaryData.consumed.carbs}</div>
          <p className="text-xs text-muted-foreground">
            {t('goal')}: {macroGoals.carbs}g
          </p>
          <Progress
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t('fat')} (g)</CardTitle>
          <Zap className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{summaryData.consumed.fat}</div>
          <p className="text-xs text-muted-foreground">
            {t('goal')}: {macroGoals.fat}g
          </p>
          <Progress
